...
import {
    parsePlansBizProWhatsIncluded,
    serializePlansBizProWhatsIncluded,
    plansBizProBulletIsEmpty,
...
describe('parsePlansBizProWhatsIncluded', () => {
    it('returns an empty model for empty input', () => {
        expect(parsePlansBizProWhatsIncluded('')).to.deep.equal({
            label: '',
            values: [],
            bullets: [],
        });
        expect(parsePlansBizProWhatsIncluded(null).bullets).to.have.lengthOf(0);
    });

    it('maps each section to one bullet, preserving title + every row', () => {
        const { bullets } = parsePlansBizProWhatsIncluded(SECTIONS_HTML);
    ...
    it('reads a merch-icon URL section icon as its src', () => {
        const html =
            ...
            '<ul><li>row</li></ul></div>';
        expect(parsePlansBizProWhatsIncluded(html).bullets[0]).to.deep.equal({
            icon: 'https://x/a.svg',
...
describe('serializePlansBizProWhatsIncluded', () => {
    ...
    it('omits the <ul> when a section has only a title', () => {
        const html = serializePlansBizProWhatsIncluded([{ icon: 'sp-icon-star', alt: '<p>Just a title</p>', link: '' }]);
    ...
    it('escapes quotes in a merch-icon src', () => {
        const html = serializePlansBizProWhatsIncluded([{ icon: 'https://x/a.svg?a="b"', alt: '<p>T</p>', link: '' }]);
        expect(html).to.contain('src="https://x/a.svg?a=&quot;b&quot;"');
    });
});

describe('plansBizProBulletIsEmpty', () => {
    it('treats icon-only bullets as non-empty', () => {
        expect(plansBizProBulletIsEmpty({ icon: 'sp-icon-star', alt: '<p></p>' })).to.equal(false);
    });
    it('treats blank title + no icon as empty', () => {
        expect(plansBizProBulletIsEmpty({ icon: '', alt: '<p></p>' })).to.equal(true);
        expect(plansBizProBulletIsEmpty({})).to.equal(true);
    });
});
...
describe('whats-included label', () => {
    ...
    it('parses a leading label element into the model label', () => {
        const model = parsePlansBizProWhatsIncluded(LABEL_HTML);
    ...
    it('parses pre-label markup with an empty label (backward compat)', () => {
        expect(parsePlansBizProWhatsIncluded(SECTIONS_HTML).label).to.equal('');
    });

    it('serializes a non-empty label ahead of the sections', () => {
        const { bullets } = parsePlansBizProWhatsIncluded(SECTIONS_HTML);
        expect(serializePlansBizProWhatsIncluded(bullets, 'See everything included:')).to.equal(LABEL_HTML);
    });

    it('escapes HTML in the label text', () => {
        const html = serializePlansBizProWhatsIncluded([{ icon: 'sp-icon-star', alt: '<p>T</p>', link: '' }], '<b>&label</b>');
    ...
    it('omits the label element when the label is blank or there are no sections', () => {
        const { bullets } = parsePlansBizProWhatsIncluded(SECTIONS_HTML);
        expect(serializePlansBizProWhatsIncluded(bullets, '  ')).to.equal(SECTIONS_HTML);
        expect(serializePlansBizProWhatsIncluded([], 'Label without sections')).to.equal('');
    });
});
...
describe('round-trip', () => {
    ...
    it('parse -> serialize reproduces labeled markup byte-identically', () => {
        ...
        const { label, bullets } = parsePlansBizProWhatsIncluded(source);
        expect(serializePlansBizProWhatsIncluded(bullets, label)).to.equal(source);
    });
});
