...
/**
 * Preview bulk publish/unpublish for multiple cards
 * Adobe I/O Runtime action for studio_preview_bulk_publish operation
 * Returns preview of what would change without executing the publish/unpublish
 */
async function main(params) {
    const { fragmentIds, action, __ow_headers } = params;
    ...
    try {
        ...
        const result = await studioOps.previewBulkPublish({ fragmentIds, action });
        ...
        console.error('Preview bulk publish error:', error);
        return {
            statusCode: 500,
