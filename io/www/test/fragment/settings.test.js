...
describe('settings', () => {
    ...
    describe('settings transformer process', () => {
        ...
        describe('hideTrialCTAs — strip behavior', () => {
            const hideTrialCTAsSetting = {
                hideTrialCTAs: {
                    default: {
                        name: 'hideTrialCTAs',
                        valuetype: 'boolean',
            ...
            const buyNowLink =
                '<a href="https://commerce.adobe.com/store/buy" data-wcs-osi="abc123" data-analytics-id="buy-now" class="accent">Buy now</a>';
            const freeTrialLink =
                '<a href="https://www.adobe.com/creativecloud/free-trial" data-wcs-osi="def456" data-analytics-id="free-trial" class="primary-outline">Free trial</a>';
            const startFreeTrialLink =
                '<a href="https://www.adobe.com/creativecloud/start-free-trial" data-wcs-osi="ghi789" data-analytics-id="start-free-trial" class="primary-outline">Start free trial</a>';

            it('dual CTA — strips free-trial link and keeps buy-now', async () => {
                const context = {
                    locale: 'en_US',
                    body: {
                        fields: {
                    ...
                    promises: { settings: Promise.resolve(hideTrialCTAsSetting) },
                };
                const result = await settings.process(context);
                expect(result.body.fields.ctas).to.equal(buyNowLink);
            });

            it('dual CTA — strips start-free-trial link and keeps buy-now', async () => {
                const context = {
                    locale: 'en_US',
                    body: {
                        fields: {
            ...
            it('single buy CTA only — unchanged when hideTrialCTAs is true', async () => {
                const context = {
                    locale: 'en_US',
                    body: {
                        fields: {
            ...
            it('single trial CTA only — results in empty string', async () => {
                const context = {
                    locale: 'en_US',
                    body: {
                        fields: {
                ...
                const result = await settings.process(context);
                expect(result.body.fields.ctas).to.equal('');
            });

            it('no CTAs (undefined) — no-op, ctas remains undefined', async () => {
                const context = {
                    locale: 'en_US',
                    body: {
                        fields: {
                ...
                const result = await settings.process(context);
                expect(result.body.fields.ctas).to.be.undefined;
            });

            it('multiple buy CTAs — both kept unchanged', async () => {
                const secondBuyNowLink =
                    '<a href="https://commerce.adobe.com/store/buy-2" data-wcs-osi="xyz999" data-analytics-id="buy-now" class="accent">Buy now</a>';
                const context = {
                    locale: 'en_US',
                    body: {
                        fields: {
            ...
            it('mixed: buy + two trial types — only buy-now link kept', async () => {
                const context = {
                    locale: 'en_US',
                    body: {
                        fields: {
            ...
            it('card-level override false — preserves trial CTA despite surface setting true', async () => {
                const context = {
                    locale: 'en_US',
                    body: {
                        fields: {
                ...
                const result = await settings.process(context);
                expect(result.body.fields.ctas).to.equal(freeTrialLink);
            });

            it('richText object: dual CTA — strips free-trial and keeps buy-now', async () => {
                const context = {
                    locale: 'en_US',
                    body: {
                        fields: {
                            variant: 'plans',
                            ctas: { mimeType: 'text/html', value: `${buyNowLink}\n${freeTrialLink}` },
                        },
                    },
                    promises: { settings: Promise.resolve(hideTrialCTAsSetting) },
                };
                const result = await settings.process(context);
                expect(result.body.fields.ctas.value).to.equal(buyNowLink);
            });

            it('richText object: single trial CTA only — value becomes empty string', async () => {
                const context = {
                    locale: 'en_US',
                    body: {
                        fields: {
                            variant: 'plans',
                            ctas: { mimeType: 'text/html', value: freeTrialLink },
                        },
                    },
                    promises: { settings: Promise.resolve(hideTrialCTAsSetting) },
                };
            ...
            it('richText object: single buy CTA only — unchanged', async () => {
                const context = {
                    locale: 'en_US',
                    body: {
                        fields: {
        ...
        describe('hideTrialCTAs — applySettings integration', () => {
            const freeTrialLink =
                '<a href="https://www.adobe.com/creativecloud/free-trial" data-wcs-osi="def456" data-analytics-id="free-trial" class="primary-outline">Free trial</a>';
            const buyNowLink =
                '<a href="https://commerce.adobe.com/store/buy" data-wcs-osi="abc123" data-analytics-id="buy-now" class="accent">Buy now</a>';

            it('surface setting true — trial CTAs stripped from fields.ctas', async () => {
                const context = {
                    locale: 'en_US',
                    body: {
                        fields: {
            ...
            it('surface setting false — fields.ctas unchanged', async () => {
                const ctas = `${buyNowLink}\n${freeTrialLink}`;
                const context = {
                    locale: 'en_US',
                    body: {
                        fields: {
                ...
                const result = await settings.process(context);
                expect(result.body.fields.ctas).to.equal(ctas);
            });

            it('surface true + card override false — trial CTAs preserved', async () => {
                const context = {
                    locale: 'en_US',
                    body: {
                        fields: {
            ...
            it('surface setting not set — fields.ctas unchanged', async () => {
                const ctas = `${buyNowLink}\n${freeTrialLink}`;
                const context = {
                    locale: 'en_US',
                    body: {
