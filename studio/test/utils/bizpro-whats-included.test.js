...
import {
    parseBizProWhatsIncluded,
    serializeBizProWhatsIncluded,
    bizProBulletIsEmpty,
} from '../../src/utils/bizpro-whats-included.js';
...
describe('parseBizProWhatsIncluded', () => {
    it('returns an empty model for empty input', () => {
        expect(parseBizProWhatsIncluded('')).to.deep.equal({
            label: '',
            values: [],
            bullets: [],
        });
        expect(parseBizProWhatsIncluded(null).bullets).to.have.lengthOf(0);
    });

    it('maps each section to one bullet, preserving title + every row', () => {
        const { bullets } = parseBizProWhatsIncluded(SECTIONS_HTML);
    ...
    it('reads a merch-icon URL section icon as its src', () => {
        const html =
            ...
            '<ul><li>row</li></ul></div>';
        expect(parseBizProWhatsIncluded(html).bullets[0]).to.deep.equal({
            icon: 'https://x/a.svg',
...
describe('serializeBizProWhatsIncluded', () => {
    ...
    it('omits the <ul> when a section has only a title', () => {
        const html = serializeBizProWhatsIncluded([{ icon: 'sp-icon-star', alt: '<p>Just a title</p>', link: '' }]);
    ...
    it('escapes quotes in a merch-icon src', () => {
        const html = serializeBizProWhatsIncluded([{ icon: 'https://x/a.svg?a="b"', alt: '<p>T</p>', link: '' }]);
        expect(html).to.contain('src="https://x/a.svg?a=&quot;b&quot;"');
    });
});

describe('bizProBulletIsEmpty', () => {
    it('treats icon-only bullets as non-empty', () => {
        expect(bizProBulletIsEmpty({ icon: 'sp-icon-star', alt: '<p></p>' })).to.equal(false);
    });
    it('treats blank title + no icon as empty', () => {
        expect(bizProBulletIsEmpty({ icon: '', alt: '<p></p>' })).to.equal(true);
        expect(bizProBulletIsEmpty({})).to.equal(true);
    });
});
...
describe('whats-included label', () => {
    ...
    it('parses a leading label element into the model label', () => {
        const model = parseBizProWhatsIncluded(LABEL_HTML);
    ...
    it('parses pre-label markup with an empty label (backward compat)', () => {
        expect(parseBizProWhatsIncluded(SECTIONS_HTML).label).to.equal('');
    });

    it('serializes a non-empty label ahead of the sections', () => {
        const { bullets } = parseBizProWhatsIncluded(SECTIONS_HTML);
        expect(serializeBizProWhatsIncluded(bullets, 'See everything included:')).to.equal(LABEL_HTML);
    });

    it('escapes HTML in the label text', () => {
        const html = serializeBizProWhatsIncluded([{ icon: 'sp-icon-star', alt: '<p>T</p>', link: '' }], '<b>&label</b>');
    ...
    it('omits the label element when the label is blank or there are no sections', () => {
        const { bullets } = parseBizProWhatsIncluded(SECTIONS_HTML);
        expect(serializeBizProWhatsIncluded(bullets, '  ')).to.equal(SECTIONS_HTML);
        expect(serializeBizProWhatsIncluded([], 'Label without sections')).to.equal('');
    });
});
...
describe('round-trip', () => {
    ...
    it('parse -> serialize reproduces labeled markup byte-identically', () => {
        ...
        const { label, bullets } = parseBizProWhatsIncluded(source);
        expect(serializeBizProWhatsIncluded(bullets, label)).to.equal(source);
    });
});
