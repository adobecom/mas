...
describe('MasSearchAndFilters', () => {
    ...
    describe('lockedTemplateFilter', () => {
        it('should preselect the locked template on connect', async () => {
            const el = await fixture(
                html`<mas-search-and-filters
                    type="cards"
                    .searchOnly=${false}
                    locked-template-filter="compare-chart"
                ></mas-search-and-filters>`,
            );
            expect(el.templateFilter).to.deep.equal(['compare-chart']);
        });

        it('should ignore template checkbox changes when locked', async () => {
            const el = await fixture(
                html`<mas-search-and-filters
                    type="cards"
                    .searchOnly=${false}
            ...
            el.templateOptions = [
                { id: 'compare-chart', title: 'Compare Chart' },
                { id: 'plans', title: 'Plans' },
            ];
            await el.updateComplete;
            const plansCheckbox = [...el.shadowRoot.querySelectorAll('sp-checkbox')].find((cb) => cb.value === 'plans');
            plansCheckbox.checked = true;
            plansCheckbox.dispatchEvent(new Event('change', { bubbles: true }));
            await el.updateComplete;
            expect(el.templateFilter).to.deep.equal(['compare-chart']);
        });

        it('should keep locked template when clear all is clicked', async () => {
            const el = await fixture(
                html`<mas-search-and-filters
                    type="cards"
                    .searchOnly=${false}
        ...
        it('should not remove locked template tag on delete', async () => {
            const el = await fixture(
                html`<mas-search-and-filters
                    type="cards"
                    .searchOnly=${false}
            ...
            el.templateOptions = [{ id: 'compare-chart', title: 'Compare Chart' }];
            await el.updateComplete;
            const tag = el.shadowRoot.querySelector('sp-tag');
            tag.value = { type: FILTER_TYPE.TEMPLATE, id: 'compare-chart' };
            tag.dispatchEvent(new CustomEvent('delete', { bubbles: true }));
            await el.updateComplete;
            expect(el.templateFilter).to.deep.equal(['compare-chart']);
        });
