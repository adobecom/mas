#!/usr/bin/env node
/**
 * Audit AOS fragment responses: validate that countries in the response
 * include US and include all supported countries.
 *
 * Usage: node scripts/aos-fragment-countries-audit/aos-fragment-countries-audit.mjs
 * Or with AOS_API_KEY: AOS_API_KEY=your_key node scripts/aos-fragment-countries-audit/aos-fragment-countries-audit.mjs
 */

import fetch from 'node-fetch';

const AOS_BASE = 'https://aos.adobe.io/offers/{fragmentId}?service_providers=PRICING&locale=en_us&country=US&api_key=aos';
const MAX_FETCH_TIMEOUT = 15000;

// Fragment IDs to audit (from provided list)
const FRAGMENT_IDS = [
...
async function fetchFragment(fragmentId, apiKey = 'aos') {
    const url = AOS_BASE.replace('{fragmentId}', fragmentId).replace('api_key=aos', `api_key=${encodeURIComponent(apiKey)}`);
...
function runAudit(apiKey = 'aos') {
    const results = [];
    return Promise.all(
        FRAGMENT_IDS.map(async (fragmentId) => {
            try {
                const data = await fetchFragment(fragmentId, apiKey);
...
function printReport(results) {
    console.log('AOS Fragment Countries Audit\n');
    console.log(
        'Endpoint pattern: https://aos.adobe.io/offers/{fragmentId}?service_providers=PRICING&locale=en_us&country=US&api_key=...\n',
    );
    ...
    for (const r of results) {
        console.log(`Fragment: ${r.fragmentId}`);
        if (!r.ok) {
