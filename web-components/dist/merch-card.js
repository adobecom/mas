...
`;var Tr={mnemonics:{size:"s"},title:{tag:"h3",slot:"heading-xxs",maxCount:250,withSuffix:!0},description:{tag:"div",slot:"body-s",maxCount:2e3,withSuffix:!1},whatsIncluded:{tag:"div",slot:"whats-included"},badge:{tag:"div",slot:"badge",default:"spectrum-yellow-300"},trialBadge:{tag:"div",slot:"trial-badge",default:"spectrum-green-800"},prices:{tag:"p",slot:"price"},ctas:{slot:"cta",size:"M"},addonConfirmation:{tag:"div",slot:"addon-confirmation"},borderColor:{attribute:"border-color",specialValues:{gray:"--spectrum-gray-300","gradient-purple-blue":"var(--gradient-purple-blue)","gradient-firefly-spectrum":"var(--gradient-firefly-spectrum)"}}},Ue=class extends f{getGlobalCSS(){return hi}get aemFragmentMapping(){return Tr}renderLayout(){return Kn`
    ...
    `);import{html as _r,nothing as Mr}from"./lit-all.min.js";import{html as Pr}from"./lit-all.min.js";function Zn(a,t){if(typeof a!="string"||!a)return null;let e;try{e=new URL(a,t)}catch{return null}return e.hostname.endsWith(".aem.page")?`${t}${e.pathname}${e.search}`:null}function Xn(a){return a?.hostname==="www.adobe.com"||a?.hostname==="adobe.com"}function Lr(a,t=globalThis.location){if(typeof a!="string"||!a||!Xn(t))return a;let e=document.createElement("template");return e.innerHTML=`<picture>${a}</picture>`,e.content.querySelectorAll("source[srcset], img[src]").forEach(r=>{let i=r.tagName==="IMG"?"src":"srcset",n=Zn(r.getAttribute(i),t.origin);n&&r.setAttribute(i,n)}),e.content.querySelector("picture").innerHTML}var Jn=new Set(["SOURCE","IMG"]),eo=new Set(["src","srcset","media","type","alt","role","loading","data-mobile-set"]);function zr(a){if(typeof a!="string"||!a)return"";let t=document.createElement("template"),e=/^\s*<picture[\s>]/i.test(a);t.innerHTML=e?a:`<picture>${a}</picture>`;let r=t.content.querySelector("picture");return r?(r.querySelectorAll("*").forEach(i=>{if(!Jn.has(i.tagName)){i.remove();return}[...i.attributes].forEach(n=>{eo.has(n.name.toLowerCase())||i.removeAttribute(n.name)})}),r.innerHTML):""}var to="(min-width: 1200px)",ro="(min-width: 600px)";function pi(a,t){if(!a)return"";let e=new DOMParser().parseFromString(`<picture>${a}</picture>`,"text/html");if(t==="desktop")return e.querySelector(`source[media="${to}"]`)?.getAttribute("srcset")??"";if(t==="tablet")return e.querySelector(`source[media="${ro}"]`)?.getAttribute("srcset")??"";if(t==="mobile"){let r=e.querySelector("img");return r?.hasAttribute("data-mobile-set")?r.getAttribute("src")??"":""}return""}var mi=["tablet","mobile"];function ao(a,t){let r=a.querySelector('[slot="backgrounds"]')?.innerHTML??"";mi.forEach(i=>{let n=t.querySelector(`[data-backgrounds-breakpoint="${i}"]`);if(!n)return;n.replaceChildren();let o=pi(r,i);if(!o){n.textContent="\u2014";return}let c=document.createElement("img");c.loading="lazy",c.alt="",c.src=o,n.append(c)})}function Ut(a){return function(e){let r=a(),i=r.shadowRoot?.querySelector(".headless-backgrounds-detail");if(!i)return;let n=i.classList.contains("hidden");n&&ao(r,i),i.classList.toggle("hidden",!n),e.target.textContent=n?"Show default only":"See all backgrounds"}}function jt(a){return Pr`
        <button
            type="button"
