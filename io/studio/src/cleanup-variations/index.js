...
async function withBackoffRetry(label, fn, shouldRetry = (r) => r.status === 429, maxRetries = 3) {
    let delay = 1000;
    for (let attempt = 0; attempt <= maxRetries; attempt++) {
        const result = await fn();
        if (!shouldRetry(result) || attempt === maxRetries) return result;
        logger.warn(`Rate limited ${label}, retrying in ${delay}ms (attempt ${attempt + 1}/${maxRetries})`);
...
async function listFragmentPaths(odinEndpoint, surface, locale, authToken) {
    ...
    do {
        ...
        const response = await withBackoffRetry(`listing ${surface}/${locale}`, () =>
            fetchOdin(odinEndpoint, `/adobe/sites/cf/fragments?${qs}`, authToken, {
...
/**
 * Fetches a fragment by path with exponential backoff retry on 429/500 responses.
 * @param {string} odinEndpoint
...
async function fetchFragmentWithRetry(odinEndpoint, fragmentPath, authToken, maxRetries = 3) {
    return withBackoffRetry(
        `fetching ${fragmentPath}`,
        () => fetchFragmentByPath(odinEndpoint, fragmentPath, authToken),
...
async function processFragmentPath(odinEndpoint, fragmentPath, surface, authToken, dryRun) {
    ...
    const validPaths = allVariationPaths.filter((vPath) => {
        const vLocale = localeFromPath(vPath);
        if (!vLocale) {
            logger.warn(`Cannot parse locale from variation path ${vPath}, keeping`);
            return true;
        }
        return validLocales.includes(vLocale);
    });
