...
const main = async ({
    ...
    const comment = async ({ pr_number, message, comments }) => {
        if (comments.some((c) => c.body.includes(message))) {
            console.log(`SNOW Transaction Comment exists. Commenting skipped... ${message}`);
            return;
        }
        process.env.LOCAL_RUN
            ? console.log(`PR #${pr_number} Local execution commenting SKIPPED message for SNOW Change Request: "${message}"`)
    ...
    try {
        ...
        if (process.env.PR_STATE !== 'open') {
            ...
            if (transactionIdComment === undefined) {
                ...
                console.log(
                    `Found SNOW Transaction ID Comment for ${path_identifier || 'default path'}. Assigning transaction ID for closing SNOW Change Request...`,
                );
            ...
            comment({
                ...
                comments,
                message: `${commentIdentifier}: ` + transaction_id,
            });
        } else {
            console.log(
                `No SNOW Transaction ID found for ${path_identifier || 'default path'}. Can't make PR comment. Skipping...`,
            );
            return;
        }
    } catch (error) {
