...
/**
 * Create a regional locale variation of a card.
 * Backs the `create_locale_variation` intent.
 *
 * The registry sends `parentId` and `locale`; StudioOperations calls the same
 * things `id` and `targetLocale`. The wire contract is the registry's, because
 * that is what the client sends, so the rename happens here.
 */
async function main(params) {
    const { parentId, locale, title, __ow_headers } = params;
    ...
    try {
        ...
        if (!locale) {
            return { statusCode: 400, body: { error: 'locale is required' } };
        }
        ...
        const result = await studioOps.createLocaleVariation({ id: parentId, targetLocale: locale, title });
        ...
        console.error('Create locale variation error:', error);
        return {
            statusCode: 500,
