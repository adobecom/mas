...
export function plansBizProBulletIsEmpty(value) {
 ...
 */
export function parsePlansBizProWhatsIncluded(html) {
    const doc = new DOMParser().parseFromString(html || '', 'text/html');
    const bullets = [];
 ...
 * parsePlansBizProWhatsIncluded; every row after the title is preserved
 ...
 * unchanged.
 */
export function serializePlansBizProWhatsIncluded(bullets, label = '') {
    const sections = (bullets ?? [])
        .filter((b) => !plansBizProBulletIsEmpty(b))
        .map(({ icon, alt }) => {
