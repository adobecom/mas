...
test.describe('M@S Studio CCD Suggested card test suite', () => {
    ...
    test(`${features[2].name},${features[2].tags}`, async ({ page, baseURL }) => {
        ...
        await test.step('step-2: Validate suggested card CSS', async () => {
            await expect(suggestedCard).toBeVisible();
            expect(
                await webUtil.verifyCSS(suggestedCard.locator(suggested.cardDescription), suggested.cssProp.description),
            ).toBeTruthy();
        });
    });
    ...
    test(`${features[3].name},${features[3].tags}`, async ({ page, baseURL }) => {
        ...
        await test.step('step-2: Validate suggested card CSS', async () => {
            await expect(suggestedCard).toBeVisible();
            expect(await webUtil.verifyCSS(suggestedCard.locator(suggested.cardIcon), suggested.cssProp.icon)).toBeTruthy();
        });
    });
});
