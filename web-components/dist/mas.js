...
var init_directive = __esm({
  "node_modules/lit-html/directive.js"() {
    ...
    e4 = (t4) => (...e6) => ({ _$litDirective$: t4, values: e6 });
    i3 = class {
      constructor(t4) {
      }
      get _$AU() {
    ...
    _MasMnemonic = class _MasMnemonic extends s4 {
      ...
      renderIcon() {
        ...
        if (this.src.startsWith("sp-icon-")) {
          return x`${o5(`<${this.src} size="${this.size || "m"}"></${this.src}>`)}`;
        }
        return x`<merch-icon
            src="${this.src}"
            size="${this.size}"
...
function normalizeMerchHeadingColumn(col) {
  ...
  const { strikethrough, price: price2, legal: legal2 } = splitPricingElement(pricingElement);
  const legalContent = [
    ...pricingAdjacentElements,
    ...legal2?.childNodes ? Array.from(legal2.childNodes) : []
  ];
  const sections = [
    createHeaderSection(
      "header-section-icon",
    ...
    createHeaderSection("header-section-price", price2 ? [price2] : []),
...
var MasTable = class extends HTMLElement {
  ...
  connectedCallback() {
    const tagName = this.localName || TAG_NAME2;
    __privateSet(this, _service5, getService2());
    __privateGet(this, _log3) ?? __privateSet(this, _log3, __privateGet(this, _service5)?.Log?.module?.(tagName) ?? __privateGet(this, _service5)?.log?.module?.(tagName) ?? console);
    this.syncDirection();
  ...
  fail(message, details = {}) {
    ...
    __privateGet(this, _log3)?.error?.(`${tagName}: ${message}`, detail);
    this.dispatchEvent(
      new CustomEvent(EVENT_MAS_ERROR, {
        bubbles: true,
        composed: true,
...
var TAG_NAME3 = "mas-comparison-table";
var MARK_PREFIX2 = "mas-comparison-table:";
var LOAD_TIMEOUT2 = 2e4;
var DEFAULT_LABELS2 = {
  "choose-table-column": "Choose table column",
  "empty-table-cell": "Empty table cell"
};
var createElement2 = (tag, attributes = {}, content = null) => {
  const element = document.createElement(tag);
  Object.entries(attributes).forEach(([key, value]) => {
    if (value == null) return;
    element.setAttribute(key, value);
  });
  appendContent2(element, content);
  return element;
};
var appendContent2 = (element, content) => {
  if (content == null) return;
  if (Array.isArray(content)) {
    content.forEach((item) => appendContent2(element, item));
    return;
  }
  if (content instanceof Node) {
    element.append(content);
...
var getStringField2 = (fields, name) => {
  const value = fields?.[name];
  if (value == null) return "";
  if (Array.isArray(value)) return String(value[0] ?? "");
  if (typeof value === "object" && "value" in value) {
...
var getArrayField2 = (fields, name) => {
  const value = fields?.[name];
  if (value == null) return [];
  return Array.isArray(value) ? value : [value];
};
var getLabels2 = (fragment) => {
  const dictionary = fragment?.dictionary || {};
  return {
    "choose-table-column": dictionary["choose-table-column"]?.value || dictionary["choose-table-column"] || DEFAULT_LABELS2["choose-table-column"],
    "empty-table-cell": dictionary["empty-table-cell"]?.value || dictionary["empty-table-cell"] || DEFAULT_LABELS2["empty-table-cell"]
  };
};
var normalizeFragmentReference = (reference) => {
  if (reference?.type === "content-fragment" && reference.value) {
    return {
      ...
      fields: normalizeFields2(reference.value)
    };
  }
  return {
    ...reference,
    path: reference?.path || "",
    fields: normalizeFields2(reference)
  };
};
...
var createSyntheticLegalPrice2 = (pricingHtml = "") => {
...
var createIconNodes = (fields) => {
  const icons = getArrayField2(fields, "mnemonicIcon");
  const alts = getArrayField2(fields, "mnemonicAlt");
  return icons.filter(Boolean).map(
    (src, index) => createElement2("mas-mnemonic", {
      src,
      size: "l",
...
var buildHeaderItem = (reference) => {
  ...
  const headerItem = createElement2("div");
  const iconNodes = createIconNodes(fields);
  if (iconNodes.length) {
    headerItem.append(
      createElement2("p", { class: "header-product-tile" }, iconNodes)
    );
  }
  const title = getStringField2(fields, "cardTitle") || reference?.title || "";
  if (title) headerItem.append(createElement2("h3", {}, title));
  headerItem.append(createElement2("p", {}, "-"));
  const descriptionHtml = getStringField2(fields, "description");
  if (descriptionHtml) {
    const description = createElement2("p");
    description.append(...cloneHtmlNodes(descriptionHtml));
    headerItem.append(description);
  }
  headerItem.append(createElement2("p", {}, "-"));
  const priceHtml = getStringField2(fields, "prices");
  if (priceHtml) {
    const pricing = createElement2("div");
    pricing.append(...cloneHtmlNodes(priceHtml));
    headerItem.append(pricing);
  }
  const legalHtml = createSyntheticLegalPrice2(priceHtml);
  if (legalHtml) {
    const legal2 = createElement2("p");
    legal2.append(...cloneHtmlNodes(legalHtml));
    headerItem.append(legal2);
  }
  const ctasHtml = getStringField2(fields, "ctas");
  if (ctasHtml) {
    ...
    buttons.forEach((button, index) => {
      const wrapper = createElement2(index === 0 ? "p" : "div", {
        class: "action-area"
      });
...
var buildSectionHeaderRow = (cardCount, title = "Features") => {
  const row = createElement2("div");
  row.append(createElement2("div", {}, title));
  for (let index = 0; index < cardCount; index += 1) {
    row.append(createElement2("div"));
  }
  return row;
};
...
var buildFeatureRow = (rowData, cardEntries) => {
  const row = createElement2("div");
  const label = createElement2("div");
  label.append(...cloneHtmlNodes(rowData.label));
  row.append(label);
  cardEntries.forEach((entry) => {
    const cell = createElement2("div");
    const value = rowData.cellsByPath?.[entry.cardRef] || rowData.cellsByPath?.[entry.path] || rowData.cellsByPath?.[entry.id] || "";
...
var calculateMaxHeight = (elements) => Math.max(
  ...elements.map((element) => {
    const styles5 = window.getComputedStyle(element);
    return element.offsetHeight - parseFloat(styles5.paddingTop) - parseFloat(styles5.paddingBottom) - parseFloat(styles5.borderTopWidth) - parseFloat(styles5.borderBottomWidth);
  })
);
  ...
  visibleSelects.forEach((select) => {
    ...
    headerTitles.forEach((title, index) => {
      if (!title || selectedIndices.has(index) && index !== currentValue)
        return;
      const option = createElement2("option", { value: index }, title);
...
var createMobileFilterSelect = ({ headerTitles, headerItemIndex, el }) => {
  const select = createElement2("select", {
    class: "mobile-filter-select",
    name: "column-filter",
    "aria-label": DEFAULT_LABELS2["choose-table-column"]
  });
  headerTitles.forEach((title, index) => {
...
var addLastContainerElements = (container) => {
  ...
  if (actionAreas.length) {
    const buttonContainer = createElement2("div", {
      class: "btn-container"
    });
  ...
  if (!description) {
    description = createElement2("p", { class: "description" });
...
var decorateHeader = (el, headerContent) => {
  ...
  const wrapper = createElement2("div", { class: "header-content-wrapper" });
  ...
  wrapper.prepend(createElement2("div", { class: "header-item" }));
  headerContent.after(
    createElement2("div", {
...
var createAccessibilityHeaderRow = (el) => {
  const headerRow = createElement2("div", {
...
var decorateTableToggleButton = ({ tableChild, tableElement }) => {
  ...
  const button = createElement2("button", { "aria-expanded": true });
  button.innerHTML = firstChild?.innerHTML || "Features";
  button.append(createElement2("span", { class: "toggle-icon" }));
  ...
  return () => {
    button.replaceWith(createElement2("div", {}, button.innerHTML));
  };
};
var processCellContent = (cell) => {
  const cellDiv = createElement2("div");
  if (cell.children.length > 1 || !cell.textContent.trim()) {
    cellDiv.append(...Array.from(cell.childNodes));
  } else {
    cellDiv.append(createElement2("p", {}, cell.innerHTML));
  }
  cell.innerHTML = "";
  cell.append(cellDiv);
};
...
var decorateSingleTable = (el, children, emptyCellLabel) => {
  const tableContainer = createElement2("div", { class: "table-container" });
  const tableElement = createElement2("div", {
    class: "table-body",
    role: "table"
  });
...
var setupResponsiveHiding = (el) => {
  ...
  const reorderElementsByColumnIndex = (elements) => {
    ...
    if (!parent) return;
    array.sort(
      (a3, b2) => parseInt(a3.getAttribute("data-column-index"), 10) - parseInt(b2.getAttribute("data-column-index"), 10)
    ).forEach((element) => {
...
var settleMasElements2 = async (root) => {
  const masElements = [...root.querySelectorAll(SELECTOR_MAS_ELEMENT)];
  await Promise.all(
    masElements.map((element) => {
      if (typeof element.onceSettled !== "function") {
...
var _cleanup2, _currentRenderId2, _content2, _log4, _resolveUpdate2, _rejectUpdate2, _service6, _startMarkName3, _durationMarkName3, _updateComplete2;
var MasComparisonTable = class extends HTMLElement {
  constructor() {
    super();
    __privateAdd(this, _cleanup2, []);
    __privateAdd(this, _currentRenderId2, 0);
    __privateAdd(this, _content2);
    __privateAdd(this, _log4);
    __privateAdd(this, _resolveUpdate2);
    __privateAdd(this, _rejectUpdate2);
    __privateAdd(this, _service6);
    __privateAdd(this, _startMarkName3);
    __privateAdd(this, _durationMarkName3);
    __privateAdd(this, _updateComplete2, Promise.resolve(this));
    this.handleAemFragmentEvents = this.handleAemFragmentEvents.bind(this);
  }
  ensureContent() {
    if (__privateGet(this, _content2)?.isConnected) return __privateGet(this, _content2);
    __privateSet(this, _content2, this.querySelector(
      ":scope > .mas-comparison-table-content"
    ));
    if (__privateGet(this, _content2)) return __privateGet(this, _content2);
    __privateSet(this, _content2, document.createElement("div"));
    __privateGet(this, _content2).className = "mas-comparison-table-content";
    this.append(__privateGet(this, _content2));
    return __privateGet(this, _content2);
  }
  connectedCallback() {
    this.ensureContent();
    __privateSet(this, _service6, getService2());
    __privateGet(this, _log4) ?? __privateSet(this, _log4, __privateGet(this, _service6)?.Log?.module?.(TAG_NAME3) ?? __privateGet(this, _service6)?.log?.module?.(TAG_NAME3) ?? console);
    const logId = this.getAttribute("id") ?? this.aemFragment?.getAttribute("fragment") ?? "unknown";
    __privateSet(this, _startMarkName3, `${MARK_PREFIX2}${logId}${MARK_START_SUFFIX}`);
    __privateSet(this, _durationMarkName3, `${MARK_PREFIX2}${logId}${MARK_DURATION_SUFFIX}`);
    performance.mark(__privateGet(this, _startMarkName3));
    this.addEventListener(EVENT_AEM_ERROR, this.handleAemFragmentEvents);
    this.addEventListener(EVENT_AEM_LOAD, this.handleAemFragmentEvents);
    this.aemFragment?.setAttribute("hidden", "");
  }
  ...
  get updateComplete() {
    return __privateGet(this, _updateComplete2);
  }
  cleanup() {
    __privateGet(this, _cleanup2).splice(0).forEach((fn) => fn?.());
  }
  beginUpdate() {
    __privateSet(this, _updateComplete2, new Promise((resolve, reject) => {
      __privateSet(this, _resolveUpdate2, resolve);
      __privateSet(this, _rejectUpdate2, reject);
    }));
  }
  async handleAemFragmentEvents(event) {
    var _a2;
    ...
    this.removeAttribute("failed");
    this.beginUpdate();
    const renderId = ++__privateWrapper(this, _currentRenderId2)._;
    try {
      await this.renderFragment(event.detail);
      if (renderId !== __privateGet(this, _currentRenderId2)) return;
      await settleMasElements2(this);
      const measure = performance.measure(
        __privateGet(this, _durationMarkName3),
        __privateGet(this, _startMarkName3)
      );
      this.dispatchEvent(
        new CustomEvent(EVENT_MAS_READY, {
          bubbles: true,
          ...
          detail: {
            ...this.aemFragment?.fetchInfo,
            ...__privateGet(this, _service6)?.duration,
            measure: printMeasure(measure)
          }
        })
      );
      (_a2 = __privateGet(this, _resolveUpdate2)) == null ? void 0 : _a2.call(this, this);
    } catch (error) {
      if (renderId !== __privateGet(this, _currentRenderId2)) return;
  ...
  async renderFragment(fragment) {
    ...
    const fields = normalizeFields2(fragment);
    const compareChart = getStringField2(fields, "compareChart").trim();
    if (!compareChart) {
      throw new Error("compareChart field is missing");
    }
    ...
    const cardEntries = getArrayField2(fields, "cards").map(String).map((cardRef) => {
    ...
    const block = createElement2("div", { class: "comparison-table" });
    const header = createElement2("div");
    cardEntries.forEach((entry) => {
    ...
    decorateTables(
      block,
      tableGroups,
      getLabels2(fragment)["empty-table-cell"]
    );
    ...
    __privateGet(this, _cleanup2).push(
      equalHeightCleanup,
      stickyCleanup,
      responsiveCleanup
    );
  ...
  fail(message, details = {}) {
    ...
    __privateGet(this, _log4)?.error?.(`${TAG_NAME3}: ${message}`, detail);
    this.dispatchEvent(
      new CustomEvent(EVENT_MAS_ERROR, {
        bubbles: true,
        composed: true,
    ...
    (_a2 = __privateGet(this, _rejectUpdate2)) == null ? void 0 : _a2.call(this, new Error(message));
  }
  async checkReady() {
    const timeoutPromise = new Promise(
      (resolve) => setTimeout(() => resolve("timeout"), LOAD_TIMEOUT2)
    );
    if (this.aemFragment) {
      const result2 = await Promise.race([
        this.aemFragment.updateComplete,
      ...
      if (result2 === false || result2 === "timeout") {
        const message = result2 === "timeout" ? `AEM fragment was not resolved within ${LOAD_TIMEOUT2} timeout` : "AEM fragment cannot be loaded";
        this.fail(message);
    ...
    if (result === "timeout") {
      const message = `${TAG_NAME3} was not resolved within ${LOAD_TIMEOUT2} timeout`;
      this.fail(message);
      throw new Error(message);
    }
    return result;
...
_currentRenderId2 = new WeakMap();
_content2 = new WeakMap();
_log4 = new WeakMap();
_resolveUpdate2 = new WeakMap();
_rejectUpdate2 = new WeakMap();
_service6 = new WeakMap();
_startMarkName3 = new WeakMap();
_durationMarkName3 = new WeakMap();
_updateComplete2 = new WeakMap();
if (!customElements.get(TAG_NAME3)) {
  customElements.define(TAG_NAME3, MasComparisonTable);
}
