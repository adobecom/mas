...
var D = (i, e, t) =>
    e in i
        ...
        : (i[e] = t);
var a = (i, e, t) => D(i, typeof e != 'symbol' ? e + '' : e, t);
import {
    html as _,
    css as B,
    LitElement as K,
...
import { html as H, LitElement as V } from './lit-all.min.js';
var Z = Object.freeze({
        MONTH: 'MONTH',
        YEAR: 'YEAR',
        TWO_YEARS: 'TWO_YEARS',
...
function u(i, e) {
    ...
    return function () {
        let o = this,
            s = arguments;
        clearTimeout(t), (t = setTimeout(() => i.apply(o, s), e));
    };
}
function T(i, e = {}, t = null, o = null) {
    let s = o
        ? document.createElement(i, { is: o })
        : document.createElement(i);
    t instanceof HTMLElement ? s.appendChild(t) : (s.innerHTML = t);
    for (let [n, r] of Object.entries(e)) s.setAttribute(n, r);
    return s;
}
function I(i) {
...
function g(i, e) {
    ...
    t.set(i, e), (window.location.hash = t.toString());
}
function A(i = []) {
    i.forEach((e) => {
...
var S = 'hashchange';
function x(i = window.location.hash) {
    let e = [],
        t = i.replace(/^#/, '').split('&');
    for (let o of t) {
        let [s, n = ''] = o.split('=');
        s && e.push([s, decodeURIComponent(n.replace(/\+/g, ' '))]);
    }
...
a(v, 'properties', { deeplink: { type: String } });
customElements.define('merch-search', v);
import {
    html as b,
    LitElement as U,
    css as Y,
...
var p = class extends U {
    ...
    selectElement(e, t = !0) {
        ...
        let o = e.querySelector('.selection');
        o?.setAttribute('selected', t);
        let s = o?.dataset,
            n = t && this.toggleIconColor ? s?.light : s?.dark;
        n && e.querySelector('img')?.setAttribute('src', n),
            t &&
                ((this.selectedElement = e),
                (this.selectedText = s?.selectedText || e.label),
    ...
    handleClick({ target: e }, t = !0) {
        let { value: o, parentNode: s } = e;
        this.selectElement(e),
            this.markCurrentItem(e),
            s?.tagName === 'SP-SIDENAV'
                ? (s
                      .querySelectorAll(
                  ...
                  s
                      ...
                      .forEach((n) => {
                          ...
                          r.value !== o && this.selectElement(r, !1);
                      }))
                : s?.tagName === 'SP-SIDENAV-ITEM' &&
                  ([
                      ...s
                          .closest('sp-sidenav')
                      ...
                      .filter((r) => r !== s)
                      .forEach((r) => {
                          (r.expanded = !1), r.removeAttribute('aria-expanded');
                      }),
                  s
                      .closest('sp-sidenav')
    ...
    startDeeplink() {
        this.stopDeeplink = l((e) => {
            let t = e[this.deeplink] ?? 'all',
                o = this.querySelector(`sp-sidenav-item[value="${t}"]`);
            o ||
                ((o = this.querySelector('sp-sidenav-item:first-child')),
                g(this.deeplink, o.value)),
                this.updateComplete.then(() => {
                    o.firstElementChild?.tagName === 'SP-SIDENAV-ITEM' &&
                        ((o.expanded = !0),
                        o.setAttribute('aria-expanded', 'true')),
                        o.parentNode?.tagName === 'SP-SIDENAV-ITEM' &&
                            ((o.parentNode.expanded = !0),
                            o.parentNode.setAttribute('aria-expanded', 'true')),
                        this.handleClick(
                            { target: o },
    ...
    connectedCallback() {
        super.connectedCallback(),
            ...
            this.updateComplete.then(() => {
                this.deeplink &&
                    (A(['filter', 'single_app']), this.startDeeplink());
            });
    }
    disconnectedCallback() {
        super.disconnectedCallback(),
...
import { html as $, LitElement as q, css as G } from './lit-all.min.js';
var m = class extends q {
    ...
    selectionChanged({ target: e }) {
        ...
        if (t) {
            let o = this.selectedValues.indexOf(t);
            e.checked && o === -1
                ? this.selectedValues.push(t)
                : !e.checked && o >= 0 && this.selectedValues.splice(o, 1);
        }
    ...
    startDeeplink() {
        this.stopDeeplink = l(({ types: e }) => {
            if (e) {
                let t = e.split(',');
                [...new Set([...t, ...this.selectedValues])].forEach((o) => {
                    let s = this.querySelector(`sp-checkbox[name=${o}]`);
                    s && (s.checked = t.includes(o));
                }),
                    (this.selectedValues = t);
            } else
                this.selectedValues.forEach((t) => {
                    let o = this.querySelector(`sp-checkbox[name=${t}]`);
                    o && (o.checked = !1);
                }),
                    (this.selectedValues = []);
        });
    ...
    render() {
        return $`<div aria-label="${this.label}">
            <div
                @change="${(e) => this.selectionChanged(e)}"
...
var N = '(max-width: 700px)',
    z = '(max-width: 767px)',
    k = '(max-width: 1199px)';
var C = '(min-width: 1200px)',
    R = '(min-width: 1600px)',
    Ce = {
        matchMobile: window.matchMedia(z),
        matchDesktop: window.matchMedia(`${C} and (not ${R})`),
        matchDesktopOrUp: window.matchMedia(C),
        matchLargeDesktop: window.matchMedia(R),
        get isMobile() {
...
var W = { catalog: 'l' },
    ...
    E = class extends K {
        constructor() {
            super();
            a(this, 'mobileDevice', new d(this, N));
            a(this, 'mobileAndTablet', new d(this, k));
            (this.open = !1),
                (this.autoclose = !1),
                (this.variant = null),
        ...
        firstUpdated() {
            let t = W[this.variant];
            if (t) {
                let s = this.querySelector('merch-search sp-search');
                s && s?.setAttribute('size', t);
            }
            let o = j[this.variant];
            o &&
                this.querySelectorAll(
                    'merch-sidenav-checkbox-group sp-checkbox',
                ).forEach((n) => {
                    n.setAttribute('size', o);
                });
        }
        updated() {
        ...
        get asDialog() {
            ...
            let t = this.autoclose
                ? L
                : _`<sp-link @click="${this.closeModal}"
        ...
        get asAside() {
            return _`<sp-theme color="light" scale="medium"
            ><h2>${this.sidenavTitle}</h2>
            <slot></slot
        ></sp-theme>`;
        }
