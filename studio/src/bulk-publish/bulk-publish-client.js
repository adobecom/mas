...
export async function publishBulk({ ioBaseUrl, paths, locales = [], token, includeVariations = false, includeCards = false }) {
    ...
    try {
        response = await fetch(`${ioBaseUrl}${ENDPOINT}`, {
            ...
            body: JSON.stringify({ paths, locales, includeVariations, includeCards }),
        });
    } catch (err) {
