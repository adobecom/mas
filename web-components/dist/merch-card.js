...
`,ir=()=>[tr`
    ...
    `);customElements.define("merch-addon",be);import{html as ot,nothing as Ri}from"./lit-all.min.js";var ye,q,qe=class qe{constructor(t){d(this,"card");g(this,ye);g(this,q);this.card=t,this.insertVariantStyle()}getContainer(){return m(this,ye,c(this,ye)??this.card.closest('merch-card-collection, [class*="-merch-cards"]')??this.card.parentElement),c(this,ye)}getHeadingObserver(){return c(this,q)?c(this,q):(m(this,q,new IntersectionObserver(([t])=>{if(t.boundingClientRect.width===0)return;let e=this.card.getBoundingClientRect().width,a=this.card.querySelector('[slot="badge"]')?.getBoundingClientRect().width||0;e===0||a===0||(this.card.style.setProperty("--consonant-merch-card-heading-xs-max-width",`${Math.round(e-a-16)}px`),c(this,q).disconnect())})),c(this,q).observe(this.card),c(this,q))}insertVariantStyle(){let t=this.constructor.name;if(!qe.styleMap[t]){qe.styleMap[t]=!0;let e=document.createElement("style");e.innerHTML=this.getGlobalCSS(),document.head.appendChild(e)}}updateCardElementMinHeight(t,e){if(!t)return;let r=`--consonant-merch-card-${this.card.variant}-${e}-height`,a=Math.max(0,parseInt(window.getComputedStyle(t).height)||0),n=parseInt(this.getContainer().style.getPropertyValue(r))||0;a>n&&this.getContainer().style.setProperty(r,`${a}px`)}get badge(){let t;if(!(!this.card.badgeBackgroundColor||!this.card.badgeColor||!this.card.badgeText))return this.evergreen&&(t=`border: 1px solid ${this.card.badgeBackgroundColor}; border-right: none;`),ot`
            <div
                id="badge"
