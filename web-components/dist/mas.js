...
`,vc=()=>[b`
    ...
    `);customElements.define("merch-icon",Gt);var Yr,Et,Xr,Kr,qt,Yi=class extends HTMLElement{constructor(){super();E(this,Yr,"");E(this,Et);E(this,Xr,[]);E(this,Kr,[]);E(this,qt);y(this,qt,zr(()=>{this.isConnected&&(this.parentElement.style.background=this.value,m(this,Et)?this.parentElement.style.borderRadius=m(this,Et):m(this,Et)===""&&(this.parentElement.style.borderRadius=""))},1))}static get observedAttributes(){return["colors","positions","angle","border-radius"]}get value(){let r=m(this,Xr).map((i,a)=>{let n=m(this,Kr)[a]||"";return`${i} ${n}`}).join(", ");return`linear-gradient(${m(this,Yr)}, ${r})`}connectedCallback(){m(this,qt).call(this)}attributeChangedCallback(r,i,a){r==="border-radius"&&y(this,Et,a?.trim()),r==="colors"&&a?y(this,Xr,a?.split(",").map(n=>n.trim())??[]):r==="positions"&&a?y(this,Kr,a?.split(",").map(n=>n.trim())??[]):r==="angle"&&y(this,Yr,a?.trim()??""),m(this,qt).call(this)}};Yr=new WeakMap,Et=new WeakMap,Xr=new WeakMap,Kr=new WeakMap,qt=new WeakMap;customElements.define("merch-gradient",Yi);M();var Vt=class extends G{constructor(){super(),this.planType=void 0,this.checked=!1,this.updatePlanType=this.updatePlanType.bind(this),this.handleChange=this.handleChange.bind(this),this.handleCustomClick=this.handleCustomClick.bind(this)}getOsi(t,r){let n=({TRIAL:["TRIAL"],BASE:["BASE","PROMOTION","TRIAL"],PROMOTION:["PROMOTION","BASE","TRIAL"]}[r]||[r]).map(s=>`p[data-plan-type="${t}"] ${D}[data-offer-type="${s}"]`).join(", ");return this.querySelector(n)?.dataset?.wcsOsi}connectedCallback(){super.connectedCallback(),this.addEventListener(Mt,this.updatePlanType)}disconnectedCallback(){super.disconnectedCallback(),this.removeEventListener(Mt,this.updatePlanType)}updatePlanType(t){if(t.target.tagName!=="SPAN")return;let r=t.target,i=r?.value?.[0];i&&(r.setAttribute("data-offer-type",i.offerType),r.closest("p").setAttribute("data-plan-type",i.planType))}handleChange(t){this.checked=t.target.checked,this.dispatchEvent(new CustomEvent("change",{detail:{checked:this.checked},bubbles:!0,composed:!0}))}handleCustomClick(){this.shadowRoot.querySelector("input").click()}handleKeyDown(t){t.key===" "&&(t.preventDefault(),this.handleCustomClick())}render(){return f` <input
                type="checkbox"
                id="addon-checkbox"
