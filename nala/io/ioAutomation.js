...
test('basic test @e2e', async ({ page }) => {
    ...
    await page.goto(url);
    const merchCardSlice = page.locator('//merch-card[@id="51c23f28-504f-450d-9764-0e60f1e279b2"]');
    await expect(merchCardSlice).toBeVisible();

    const merchIcon = merchCardSlice.locator('//merch-icon');
    ...
    const imageDiv = merchCardSlice.locator('div[slot="image"]');
    ...
    const priceSpan = merchCardSlice.locator('span[class="price"]');
    await expect(priceSpan).toBeVisible({ timeout: 15000 });

    const currencySymbol = priceSpan.locator('span[class="price-currency-symbol"]');
    await expect(currencySymbol).toBeVisible();

    const priceInteger = priceSpan.locator('span[class="price-integer"]');
    await expect(priceInteger).toBeVisible();

    const decimalsDelimiter = priceSpan.locator('span[class="price-decimals-delimiter"]');
    await expect(decimalsDelimiter).toBeVisible();

    const priceDecimals = priceSpan.locator('span[class="price-decimals"]');
    await expect(priceDecimals).toBeVisible();

    const priceRecurrence = priceSpan.locator('span[class="price-recurrence"]');
    ...
    const termsLink = merchCardSlice.locator('a[data-analytics-id="see-terms"]');
    await expect(termsLink).toBeVisible();
    const href = await termsLink.getAttribute('href');
    expect(href).toBeTruthy();
    expect(href.length).toBeGreaterThan(0);

    const checkoutButton = merchCardSlice.locator('button[is="checkout-button"]');
    ...
    const merchCardSuggested = page.locator('//merch-card[@id="549f6981-f5c8-4512-b41c-313d60f375b2"]');
    await merchCardSuggested.scrollIntoViewIfNeeded();
    await expect(merchCardSuggested).toBeVisible();
    const merchIconSuggested = merchCardSuggested.locator('//merch-icon');
    ...
    const headingXs = merchCardSuggested.locator('h3[slot="heading-xs"]');
    await expect(headingXs).toBeVisible();

    const detailS = merchCardSuggested.locator('h4[slot="detail-s"]');
    await expect(detailS).toBeVisible();

    const bodyXS = merchCardSuggested.locator('div[slot="body-xs"]');
    await expect(bodyXS).toBeVisible();

    const bodyText = bodyXS.locator('p');
    await expect(bodyText).toBeVisible();
    const text = await bodyText.textContent();
    expect(text).toBeTruthy();
    expect(text.length).toBeGreaterThan(0);

    const bodyLink = bodyXS.locator('a');
    ...
    const priceSlotSuggested = merchCardSuggested.locator('p[slot="price"]');
    await expect(priceSlotSuggested).toBeVisible();

    // Check strikethrough price span
    const strikethroughPrice = priceSlotSuggested.locator('span[data-template="strikethrough"]');
    await expect(strikethroughPrice).toBeVisible();

    const priceCurrencySymbolStrike = strikethroughPrice.locator('span[class="price-currency-symbol"]');
    await expect(priceCurrencySymbolStrike).toBeVisible();

    const priceIntegerStrike = strikethroughPrice.locator('span[class="price-integer"]');
    await expect(priceIntegerStrike).toBeVisible();

    const priceDecimalsDelimiterStrike = strikethroughPrice.locator('span[class="price-decimals-delimiter"]');
    await expect(priceDecimalsDelimiterStrike).toBeVisible();

    const priceDecimalsStrike = strikethroughPrice.locator('span[class="price-decimals"]');
    await expect(priceDecimalsStrike).toBeVisible();

    const priceRecurrenceStrike = strikethroughPrice.locator('span[class="price-recurrence"]');
    await expect(priceRecurrenceStrike).toBeVisible();

    // Check regular price span
    const regularPrice = priceSlotSuggested.locator('span[data-template="price"]');
    await expect(regularPrice).toBeVisible();

    const priceCurrencySymbolRegular = regularPrice.locator('span[class="price-currency-symbol"]');
    await expect(priceCurrencySymbolRegular).toBeVisible();

    const priceIntegerRegular = regularPrice.locator('span[class="price-integer"]');
    await expect(priceIntegerRegular).toBeVisible();

    const priceDecimalsDelimiterRegular = regularPrice.locator('span[class="price-decimals-delimiter"]');
    await expect(priceDecimalsDelimiterRegular).toBeVisible();

    const priceDecimalsRegular = regularPrice.locator('span[class="price-decimals"]');
    await expect(priceDecimalsRegular).toBeVisible();

    const priceRecurrenceRegular = regularPrice.locator('span.price-recurrence');
    await expect(priceRecurrenceRegular).toBeVisible();

    // Check checkout button href
    const checkoutButtonSuggested = merchCardSuggested.locator('button[is="checkout-button"]');
