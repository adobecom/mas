...
/**
 * AEM fragment field → slot mapping so hydrate() can populate all FAQ Headless slots.
 ...
 */
export const FAQ_HEADLESS_AEM_FRAGMENT_MAPPING = {
...
/**
 * Slot name to display label for FAQ Headless variant (label + value only, no card).
 * Labels match the editor (merch-card-editor.js). Order defines render order.
 */
const FAQ_HEADLESS_FIELDS = [
...
export class FaqHeadless extends VariantLayout {
    constructor(card) {
        super(card);
    }
    ...
    static variantStyle = headlessRowStyle('faq-headless');
}
