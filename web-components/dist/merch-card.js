...
`;var Ai=x,Ti=["heading-s","body-xs","heading-xs"],Li={cardName:{attribute:"name"},mnemonics:{size:"s"},badge:{tag:"div",slot:"badge"},title:{tag:"h3",slot:"heading-s"},prices:{tag:"p",slot:"heading-xs"},description:{tag:"div",slot:"body-xs"},ctas:{slot:"footer",size:"m"},style:"consonant"},ne,Ct,Ze=class extends f{constructor(){super(...arguments);v(this,ne,null);v(this,Ct,()=>this.resyncOnReflow());d(this,"lastSyncKey",null)}getGlobalCSS(){return Si}priceOptionsProvider(e,t){e.dataset.template===P&&(t.displayPlanType=!0)}async adjustLegal(){if(!this.legalAdjusted)try{this.legalAdjusted=!0,await this.card.updateComplete,await customElements.whenDefined("inline-price");let e=this.card.querySelector(`[slot="heading-xs"] ${w}:not([data-template="legal"])`);if(!e)return;let t=e.cloneNode(!0);if(await e.onceSettled(),!e.options)return;t.setAttribute("data-template","legal"),t.dataset.displayPerUnit="false",e.parentNode.insertBefore(t,e.nextSibling),await t.onceSettled()}catch{}}async postCardUpdateHook(){this.card.isConnected&&(this.legalAdjusted||await this.adjustLegal(),await super.postCardUpdateHook(),this.flagPriceRow(),window.matchMedia(Ai).matches&&requestAnimationFrame(()=>this.syncHeights()))}syncHeights(){this.card.getBoundingClientRect().width<=2||window.matchMedia(Ai).matches&&this.syncRowHeights(Ti.map(e=>({name:e,getElement:t=>t.querySelector(`[slot="${e}"]`)})))}flagPriceRow(){this.card.toggleAttribute("no-price",!this.card.querySelector('[slot="heading-xs"]'))}resyncOnReflow(){let e=this.card.getBoundingClientRect().width;if(e<=2)return;let t=n=>Math.round(this.card.querySelector(n)?.getBoundingClientRect().height||0),i=[Math.round(e),...Ti.map(n=>t(`[slot="${n}"]`))].join(":");i!==this.lastSyncKey&&(this.lastSyncKey=i,this.syncHeights())}connectedCallbackHook(){if(this.card.addEventListener(N,p(this,Ct)),typeof ResizeObserver>"u")return;g(this,ne,new ResizeObserver(()=>this.resyncOnReflow())),p(this,ne).observe(this.card);let e=this.card.querySelector('[slot="body-xs"]');e&&p(this,ne).observe(e)}disconnectedCallbackHook(){this.card.removeEventListener(N,p(this,Ct)),p(this,ne)?.disconnect(),g(this,ne,null)}renderLayout(){return so` <div class="header">
                <slot name="icons"></slot>
                <slot name="badge"></slot>
            </div>
            <div class="panel">
