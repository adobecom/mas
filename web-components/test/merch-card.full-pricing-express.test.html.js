    ...
    describe('merch-card full-pricing-express variant', () => {
        describe('Basic Structure', () => {
            ...
            it('should render with correct shadow DOM structure', async () => {
                ...
                const priceContainer =
                    shadowRoot.querySelector('.price-container');
                ...
                const shortDescription =
                    shadowRoot.querySelector('.short-description');
        ...
        describe('Slot Content', () => {
            ...
            it('should support mnemonic icon in heading', async () => {
                ...
                cards.forEach((card) => {
                    const mnemonic = card.querySelector(
                        '[slot="heading-xs"] mas-mnemonic',
                    );
                    if (mnemonic) {
                        foundMnemonic = true;
            ...
            it('should have price slot with inline-price', async () => {
                ...
                const inlinePrice = price.querySelector(
                    'span[is="inline-price"]',
                );
                expect(inlinePrice).to.exist;
                expect(inlinePrice.getAttribute('data-template')).to.equal(
                    'price',
                );
            });
        ...
        describe('Short Description Slot with Dividers', () => {
            it('should have description2 slot with content', async () => {
                ...
                const description2 = card.querySelector(
                    '[slot="description2"]',
                );
            ...
            it('should have Top features label', async () => {
                ...
                const firstParagraph =
                    description2.querySelector('p:first-child');
            ...
            it('should have divider-wrapper elements', async () => {
                ...
                const dividerWrappers =
                    description2.querySelectorAll('.divider-wrapper');
            ...
            it('should have Compare all features link at bottom', async () => {
                ...
                const buttonContainer =
                    description2.querySelector('.button-container');
                expect(buttonContainer).to.exist;

                const compareLink = buttonContainer.querySelector('a');
                expect(compareLink).to.exist;
                expect(compareLink.textContent).to.equal(
                    'Compare all features',
                );
                expect(compareLink.classList.contains('spectrum-Link')).to.be
                    .true;
            });
        });
            ...
            it('should support badge slot when provided', async () => {
                ...
                cards.forEach((card) => {
                    ...
                    if (badge) {
                        ...
                        if (merchBadge) {
                            expect(merchBadge.getAttribute('variant')).to.equal(
        ...
        describe('Gradient Border Support', () => {
            ...
            it('should apply gradient-firefly-spectrum border color', async () => {
                const card = document.querySelector(
                    'merch-card[border-color="gradient-firefly-spectrum"]',
                );
                if (card) {
                    expect(card.getAttribute('gradient-border')).to.equal(
                        'true',
                    );
                }
            });
        ...
        describe('Collection Support', () => {
            ...
            it('collection should have correct class', async () => {
                ...
                expect(collection.classList.contains('full-pricing-express')).to
                    .be.true;
            });
            ...
            it('collection should contain multiple cards', async () => {
                ...
                const cards = collection.querySelectorAll(
                    'merch-card[variant="full-pricing-express"]',
                );
        ...
        describe('Price Display', () => {
            it('should render main price correctly', async () => {
                ...
                const firstPrice = priceSlot.querySelector(
                    'p:first-child span[is="inline-price"]',
                );
                ...
                const currencySymbol = priceElement.querySelector(
                ...
                const priceInteger =
                    priceElement.querySelector('.price-integer');
                expect(priceInteger).to.exist;

                const priceRecurrence =
                    priceElement.querySelector('.price-recurrence');
                if (
                    priceRecurrence &&
                    !priceRecurrence.classList.contains('disabled')
                ) {
            ...
            it('should support price without recurrence display', async () => {
                ...
                cards.forEach((card) => {
                    const priceSpan = card.querySelector(
                        'span[is="inline-price"][data-display-recurrence="false"]',
                    );
                    if (priceSpan) {
                        foundNoRecurrence = true;
                        expect(
                            priceSpan.getAttribute('data-display-recurrence'),
                        ).to.equal('false');
                    }
                });
        ...
        describe('Alignment Mechanism', () => {
            it('should have min-height CSS variables for alignment', async () => {
                ...
                const priceHeight = computedStyle.getPropertyValue(
                    '--consonant-merch-card-full-pricing-express-price-height',
                );
                const ctaHeight = computedStyle.getPropertyValue(
                    '--consonant-merch-card-full-pricing-express-cta-height',
                );
                const shortDescriptionHeight = computedStyle.getPropertyValue(
                ...
                const descriptionHeight = computedStyle.getPropertyValue(
                    '--consonant-merch-card-full-pricing-express-description-height',
                );
            ...
            it('should apply flexbox to short-description for alignment', async () => {
                ...
                if (mediaQuery.matches) {
                    const computedStyle =
                        window.getComputedStyle(shortDescription);
        ...
        describe('Analytics Integration', () => {
            ...
            it('should have daa-ll attribute on links', async () => {
                ...
                const compareLink = card.querySelector(
                    '[slot="description2"] a',
                );
