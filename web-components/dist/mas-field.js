...
`;if(typeof document<"u"&&!document.querySelector("style[data-plan-type-text]")){let e=document.createElement("style");e.setAttribute("data-plan-type-text",""),e.textContent=gt,document.head.append(e)}var St="p, div, li, td, th, h1, h2, h3, h4, h5, h6, section, article, blockquote";function Ct(e){let r=e.closest(St)??e.parentNode,t=document.createRange();return t.setStart(r,0),t.setEndBefore(e),t.toString().replace(/\s+$/,"").slice(-1)}function nt(e){let r=[...e.querySelectorAll('[is="inline-price"][data-template="price"]')].filter(i=>!i.closest("merch-addon"));return(r.find(i=>i.dataset.promotionCode&&i.dataset.promotionCode!=="cancel-context")??r[0])?.dataset.wcsOsi??e.aemFragment?.data?.fields?.osi}function Lt(e){let r=Ct(e);return!r||_t.includes(r)?"upper":"lower"}function Y(e,r){if(e.dataset.placeholder!==J)return;let t=e.closest("merch-card, mas-field")?.osi;t&&(r.wcsOsi=t,r.planTypeCase=Lt(e))}var K="mas-field",xt=/(accent|primary|secondary)(-(outline|link))?/,bt=["fragment-id","variation-id","mask-id","data-promotion-project","data-promotion-variation-project"];function q(e){return e.compatVersion>=1||e.hasAttribute("data-promotion-project")?e.getAttribute("data-promotion-code"):null}function rt(e,r){let t=document.createElement("template");t.innerHTML=e;let i=[...t.content.querySelectorAll("a")],o=i.filter(n=>z.has(n.dataset.analyticsId));return o.length===0?e:o.length===i.length?r?null:e:(o.forEach(n=>n.remove()),t.innerHTML)}function it(e,r){if(!e)return r;let t=e.closest(K);if(!(t||e.hasAttribute("fragment-id")))return r;r[tt]=!0,r.wrapClauses=!0;let o=t?.aemFragment?.data?.priceLiterals;if(o&&(r.literals??(r.literals={}),Object.assign(r.literals,o)),ot(e)&&(r.displayPerUnit=!1,r.displayTax=!1),t&&e.dataset.template===Q&&(r.displayPlanType=t.aemFragment?.data?.settings?.displayPlanType??!1),!r.promotionCode){let n=e.dataset.promotionCode??(t?q(t):null);n&&(r.promotionCode=n)}r.displayAnnual===void 0&&typeof t?.settings?.displayAnnual=="boolean"&&(r.displayAnnual=t.settings.displayAnnual)}function Nt(e,r){if(r.promotionCode||!e)return;let t=e.closest(K),i=e.dataset.promotionCode??(t?q(t):null);i&&(r.promotionCode=i)}function Ot(e){!e?.providers||e.providers.has(it)||(e.providers.price(it),e.providers.checkout(Nt),e.providers.has(Y)||e.providers.price(Y))}var Mt=`
mas-field {
    display: contents;
}
