...
var MASLIBS_PATTERN = /^([a-z0-9]+(-[a-z0-9]+)*)(--([a-z0-9]+(-[a-z0-9]+)*)){0,2}$/;
var MASLIBS_MAX_LENGTH = 100;
var MASLIBS_EXTENSIONS = ["live", "page"];
function getValidatedMasLibsUrl(masLibs, extension = "live") {
...
// ../io/www/src/fragment/utils/explicit-empty.js
var EXPLICIT_EMPTY_SENTINEL = "<explicit_empty/>";
function isExplicitEmptySentinel(value) {
  return value === EXPLICIT_EMPTY_SENTINEL;
}
function fieldValuesArePersistedExplicitEmpty(values) {
  return Array.isArray(values) && values.length === 1 && isExplicitEmptySentinel(values[0]);
}
function normalizeExplicitEmptyInFields(fields) {
  ...
  for (const [fieldName, fieldValue] of Object.entries(fields)) {
    if (Array.isArray(fieldValue)) {
      result[fieldName] = fieldValuesArePersistedExplicitEmpty(fieldValue) ? [] : fieldValue.map((v) => isExplicitEmptySentinel(v) ? "" : v);
...
var _log, _rawData, _data, _service3, _fragmentId, _fetchInfo, _loading, _mask, _pzn, _timeout, _fetchPromise, _author, _fetchCount, _preview, _AemFragment_instances, getFragment_fn, applyHeaders_fn, fail_fn, fetchData_fn;
var AemFragment = class extends HTMLElement {
  ...
  connectedCallback() {
    ...
    __privateSet(this, _service3, getService2(this));
    if (!__privateGet(this, _service3)?.settings) {
  ...
  transformAuthorData() {
    ...
    __privateSet(this, _data, normalizedFields.reduce(
  ...
  getFragmentClientUrl() {
    ...
    const baseUrl = getValidatedMasLibsUrl(urlParams.get("maslibs"), extension) ?? "https://mas.adobe.com";
  ...
  async generatePreview() {
    ...
    const instant = new URLSearchParams(window.location.search).get("instant") ?? __privateGet(this, _service3).settings.instant;
    const options = {
      ...defaultOptions,
...
fetchData_fn = async function() {
  ...
  const {
    ...
    locale,
    instant
  } = __privateGet(this, _service3).settings;
...
function contextPromotionCode(masField) {
  if (masField.compatVersion >= COMPAT_VERSION_GLOBAL_PROMO_CODE || masField.hasAttribute("data-promotion-project"))
...
function stripTrialCtas(html, indexed) {
  ...
  const trials = anchors.filter(
    (anchor) => TRIAL_ANALYTICS_IDS.has(anchor.dataset.analyticsId)
  );
...
function priceOptionsProvider(element, options) {
  ...
  options.wrapClauses = true;
  const priceLiterals2 = masField?.aemFragment?.data?.priceLiterals;
  if (priceLiterals2) {
    options.literals ?? (options.literals = {});
    Object.assign(options.literals, priceLiterals2);
  }
  ...
  if (masField && element.dataset.template === TEMPLATE_PRICE_LEGAL) {
    options.displayPlanType = masField.aemFragment?.data?.settings?.displayPlanType ?? false;
  }
  if (!options.promotionCode) {
    const promotionCode = element.dataset.promotionCode ?? (masField ? contextPromotionCode(masField) : null);
    if (promotionCode) options.promotionCode = promotionCode;
  }
  if (options.displayAnnual === void 0 && typeof masField?.settings?.displayAnnual === "boolean") {
...
var _field, _loaded, _fields, _contentElement, _onFragmentLoad, _MasField_instances, ensureContentElement_fn, normalizeFieldValue_fn, parseFieldAndIndex_fn, extractIndexedAnchor_fn, setFragmentIds_fn, renderField_fn, decorateTooltips_fn, positionTooltip_fn, stampContext_fn, buildCtaButton_fn, renderCtaField_fn, unwrapSingleParagraph_fn;
var MasField = class extends HTMLElement {
  ...
  connectedCallback() {
    ...
    registerOptionsProviders(getService2());
  }
...
renderField_fn = function() {
  ...
  if (index !== null && isNaN(index)) {
    ...
    const labelsRaw = __privateGet(this, _fields)[labelsFieldName];
    if (labelsRaw !== void 0) {
      const labels = Array.isArray(labelsRaw) ? labelsRaw : [labelsRaw];
      const labelIndex = labels.indexOf(index);
      if (labelIndex === -1) return;
      const valuesRaw = __privateGet(this, _fields)[fieldName];
      const values = Array.isArray(valuesRaw) ? valuesRaw : valuesRaw ? [valuesRaw] : [];
      let html2 = __privateMethod(this, _MasField_instances, normalizeFieldValue_fn).call(this, values[labelIndex]);
      if (!html2) return;
      if (fieldName === "ctas" && this.settings?.hideTrialCTAs) {
        html2 = stripTrialCtas(html2, true);
        if (html2 === null) return;
      }
      __privateMethod(this, _MasField_instances, setFragmentIds_fn).call(this);
      const content2 = __privateMethod(this, _MasField_instances, ensureContentElement_fn).call(this);
      content2.innerHTML = __privateMethod(this, _MasField_instances, unwrapSingleParagraph_fn).call(this, html2) ?? "";
      __privateMethod(this, _MasField_instances, decorateTooltips_fn).call(this, content2);
      __privateMethod(this, _MasField_instances, stampContext_fn).call(this, content2);
      return;
    }
  }
  ...
  if (typeof html === "string") {
    ...
    if (__privateGet(this, _field) === "ctas") {
      ...
      if (ctaEl) {
        ...
        __privateMethod(this, _MasField_instances, stampContext_fn).call(this, content);
        return;
      }
    }
    content.innerHTML = html;
    __privateMethod(this, _MasField_instances, decorateTooltips_fn).call(this, content);
    __privateMethod(this, _MasField_instances, stampContext_fn).call(this, content);
    return;
  }
 ...
 */
decorateTooltips_fn = function(root) {
  ...
  for (const icon of icons) {
    ...
    const authored = [...icon.classList].find(
      (c) => placements.includes(c)
    );
    ...
    const show = () => {
      ...
      __privateMethod(this, _MasField_instances, positionTooltip_fn).call(this, icon);
    };
 ...
 */
positionTooltip_fn = function(tooltip) {
  ...
  const width = px(before.width) + px(before.paddingLeft) + px(before.paddingRight);
  const height = px(before.height) + px(before.paddingTop) + px(before.paddingBottom);
  ...
  const effectiveHeight = original === "top" ? height + topMargin : height / 2;
  const willCutoffTop = rect.top - effectiveHeight < headerHeight;
  const willCutoffBottom = rect.bottom + (original === "bottom" ? height + margin : 0) > window.innerHeight;
  const willOverflowRight = rect.right + effectiveMaxWidth + margin > viewportWidth;
  const willOverflowLeft = rect.left - effectiveMaxWidth - margin < 0;
  const willOverflowRightAtBottom = rect.left + width / 2 + margin > viewportWidth;
  const willOverflowLeftAtBottom = rect.left - width / 2 - margin < 0;
  const hasOverflow = willOverflowRight || willOverflowLeft || willCutoffTop || willCutoffBottom || willOverflowRightAtBottom || willOverflowLeftAtBottom;
  ...
  if (willOverflowRight && willOverflowRightAtBottom) {
    ...
    updated = willOverflowRightAtBottom && "left" || willOverflowLeftAtBottom && "right" || "bottom";
  } else if (willOverflowRight !== willOverflowLeft && !willCutoffBottom) {
    updated = willOverflowRight ? "left" : "right";
  } else if (willCutoffTop && ["top", "left", "right"].includes(original)) {
    updated = "bottom";
  } else if (willCutoffBottom && ["bottom", "left", "right"].includes(original)) {
    updated = "top";
  }
  if (current !== updated) {
    tooltip.classList.remove(...placements);
 ...
 */
stampContext_fn = function(content) {
...
buildCtaButton_fn = function(link) {
  ...
  const button = CheckoutLink2?.createCheckoutLink(link.dataset, link.textContent) ?? (() => {
...
function buildImage(mnemonicIcon) {
  if (!Array.isArray(mnemonicIcon) || mnemonicIcon.length === 0)
    return void 0;
  const suffix2 = "?format=png&width=800&height=800";
  if (mnemonicIcon.length === 1) return `${mnemonicIcon[0]}${suffix2}`;
  return mnemonicIcon.map((icon) => `${icon}${suffix2}`);
}
...
function injectJsonLd(fields, offer, regularOffer, pageUrl) {
  ...
  const { price: price2 } = offer?.priceDetails ?? {};
  ...
  if (effectiveRegularPrice != null && offer?.priceDetails?.priceWithoutDiscount == null && effectiveRegularPrice < effectivePrice) {
  ...
  if (effectiveRegularPrice != null && effectiveRegularPrice !== effectivePrice) {
  ...
  const description = fields.description?.value ? stripHtml(fields.description.value) : void 0;
  const jsonLd = {
