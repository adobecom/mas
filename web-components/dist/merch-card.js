...
`;var oa={title:{tag:"p",slot:"title"},prices:{tag:"p",slot:"prices"},description:{tag:"p",slot:"description"},planType:!0,ctas:{slot:"ctas",size:"S"}},Me=class extends x{constructor(){super(...arguments);d(this,"legal")}async postCardUpdateHook(){this.card.isConnected&&(await this.card.updateComplete,this.adjustLegal(),this.card.prices?.length>0&&await Promise.all(this.card.prices.map(e=>e.onceSettled?.()||Promise.resolve())))}getGlobalCSS(){return na}get headingSelector(){return'[slot="title"]'}priceOptionsProvider(e,r){r.literals={...r.literals,strikethroughAriaLabel:"",alternativePriceAriaLabel:""},r.space=!0,r.displayAnnual=this.card.settings?.displayAnnual??!1}adjustLegal(){if(this.legal!==void 0)return;let e=this.card.querySelector(`${b}[data-template="price"]`);if(!e)return;let r=e.cloneNode(!0);this.legal=r,e.dataset.displayTax="false",e.dataset.displayPerUnit="false",r.dataset.template="legal",r.dataset.displayPlanType=this.card?.settings?.displayPlanType??!0,r.setAttribute("slot","legal"),this.card.appendChild(r)}renderLayout(){return Ai`
            ${this.badge}
            <div class="body">
