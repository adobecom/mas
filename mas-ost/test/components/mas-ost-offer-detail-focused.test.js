...
import '../../src/components/mas-ost-offer-detail-focused.js';
...
describe('mas-ost-offer-detail-focused', () => {
    beforeEach(() => {
        resetStore();
    });
    ...
    it('renders empty when no offer is selected', async () => {
        const el = await fixture(html`<mas-ost-offer-detail-focused></mas-ost-offer-detail-focused>`);
    ...
    it('shows an OSI empty placeholder when no OSI has been resolved', async () => {
        store.selectedOffer = makeOffer();
        const el = await fixture(html`<mas-ost-offer-detail-focused></mas-ost-offer-detail-focused>`);
        // mas-ost-offer-detail-focused.ensureOsi may race; force the visible state for assertion.
        el.osi = '';
        el.osiError = '';
        el.osiLoading = false;
        await el.updateComplete;
