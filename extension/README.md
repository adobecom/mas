...
There is none, by design. Fragment data comes from the public M@S IO endpoint using the public `wcms-commerce-ims-user-prod` API key, so the extension reads only what is already published. It requests no Adobe credentials and holds no tokens.
...
```
GET https://www.adobe.com/mas/io/fragment?id={fragmentId}&api_key=wcms-commerce-ims-user-prod&locale={locale}
```
