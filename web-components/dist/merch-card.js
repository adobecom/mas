...
`;function ae(a){let r=Bn(a);return In`
    ...
    `}import{html as zr}from"./lit-all.min.js";function Hn(a,r){if(typeof a!="string"||!a)return null;let e;try{e=new URL(a,r)}catch{return null}return e.hostname.endsWith(".aem.page")?`${r}${e.pathname}${e.search}`:null}function qn(a){return a?.hostname==="www.adobe.com"||a?.hostname==="adobe.com"}function Tr(a,r=globalThis.location){if(typeof a!="string"||!a||!qn(r))return a;let e=document.createElement("template");return e.innerHTML=`<picture>${a}</picture>`,e.content.querySelectorAll("source[srcset], img[src]").forEach(t=>{let i=t.tagName==="IMG"?"src":"srcset",n=Hn(t.getAttribute(i),r.origin);n&&t.setAttribute(i,n)}),e.content.querySelector("picture").innerHTML}var Un=new Set(["SOURCE","IMG"]),jn=new Set(["src","srcset","media","type","alt","loading","data-mobile-set"]);function Lr(a){if(typeof a!="string"||!a)return"";let r=document.createElement("template");r.innerHTML=`<picture>${a}</picture>`;let e=r.content.querySelector("picture");return e?(e.querySelectorAll("*").forEach(t=>{if(!Un.has(t.tagName)){t.remove();return}[...t.attributes].forEach(i=>{jn.has(i.name.toLowerCase())||t.removeAttribute(i.name)})}),e.innerHTML):""}var Gn="(min-width: 1200px)",Vn="(min-width: 600px)";function ni(a,r){if(!a)return"";let e=new DOMParser().parseFromString(`<picture>${a}</picture>`,"text/html");if(r==="desktop")return e.querySelector(`source[media="${Gn}"]`)?.getAttribute("srcset")??"";if(r==="tablet")return e.querySelector(`source[media="${Vn}"]`)?.getAttribute("srcset")??"";if(r==="mobile"){let t=e.querySelector("img");return t?.hasAttribute("data-mobile-set")?t.getAttribute("src")??"":""}return""}var oi=["tablet","mobile"];function Wn(a,r){let t=a.querySelector('[slot="backgrounds"]')?.innerHTML??"";oi.forEach(i=>{let n=r.querySelector(`[data-backgrounds-breakpoint="${i}"]`);if(!n)return;n.replaceChildren();let o=ni(t,i);if(!o){n.textContent="\u2014";return}let c=document.createElement("img");c.loading="lazy",c.alt="",c.src=o,n.append(c)})}function Be(a){return function(e){let t=a(),i=t.shadowRoot?.querySelector(".headless-backgrounds-detail");if(!i)return;let n=i.classList.contains("hidden");n&&Wn(t,i),i.classList.toggle("hidden",!n),e.target.textContent=n?"Show default only":"See all backgrounds"}}function He(a){return zr`
        <button
            type="button"
