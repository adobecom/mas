...
test.describe('M@S Studio CCD Slice card test suite', () => {
    ...
    test(`${features[2].name},${features[2].tags}`, async ({ page, baseURL }) => {
        ...
        await test.step('step-9: Validate all field changes in parallel', async () => {
            ...
            if (failures.length > 0) {
                throw new Error(`❌ Slice card field save validation failures:\n${failures.join('\n')}`);
            }
        });
    });
    ...
    test(`${features[4].name},${features[4].tags}`, async ({ page, baseURL }) => {
        ...
        await test.step('step-9: Validate all CTA changes in parallel', async () => {
            ...
            if (failures.length > 0) {
                throw new Error(`❌ Slice card CTA link save validation failures:\n${failures.join('\n')}`);
            }
        });
    });
    ...
    test(`${features[5].name},${features[5].tags}`, async ({ page, baseURL }) => {
        ...
        await test.step('step-4: Validate osi change', async () => {
            ...
            await expect(await editor.tags).toHaveAttribute('value', new RegExp(`${data.osiTags.new.offerTypeTag}`));
            await expect(await editor.tags).toHaveAttribute('value', new RegExp(`${data.osiTags.new.marketSegmentsTag}`));
            await expect(await editor.tags).toHaveAttribute('value', new RegExp(`${data.osiTags.new.planTypeTag}`));
