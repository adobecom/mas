...
import './mas-create-variation-dialog.js';
...
export default class MasFragmentEditor extends LitElement {
    ...
    static properties = {
        ...
        variationCreationLoading: { type: Boolean, state: true },
    ...
    constructor() {
        ...
        this.confirmCreateVariation = this.confirmCreateVariation.bind(this);
        this.cancelCreateVariation = this.cancelCreateVariation.bind(this);
    }
    ...
    async showCreateVariation() {
        this.showCreateVariationDialog = true;
    }

    async confirmCreateVariation(event) {
        const { locale } = event.detail;
        const dialog = this.querySelector('mas-create-variation-dialog');

        this.variationCreationLoading = true;
        try {
            const newFragmentId = await this.repository.createFragmentVariation(this.fragment, locale);
            ...
            showToast(`Failed to create variation: ${error.message}`, 'negative');
            this.variationCreationLoading = false;
        }
    }
    ...
    get createVariationDialog() {
        if (!this.showCreateVariationDialog) return nothing;
        return html`
            <mas-create-variation-dialog
                .fragment=${this.fragment}
                .loading=${this.variationCreationLoading}
                @create-variation=${this.confirmCreateVariation}
    ...
    get fragmentEditor() {
        ...
        let sectionTitle = 'Fragment Editor';

        switch (this.fragment.model.path) {
            case CARD_MODEL_PATH:
                sectionTitle = 'Fragment Editor';
                editorContent = html`
                    <merch-card-editor
            ...
            case COLLECTION_MODEL_PATH:
                sectionTitle = 'Collection Editor';
                editorContent = html`
