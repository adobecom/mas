...
import { getParentReference } from './common.js';
...
test('regional locale points to surface default', () => {
    ...
    assert.equal(
        getParentReference('express', 'fr_CA'),
        `${ROOT}/express/fr_FR/dictionary/index`,
    );
});
...
test('surface default locale points to acom default', () => {
    ...
    assert.equal(
        getParentReference('express', 'fr_FR'),
        `${ROOT}/acom/fr_FR/dictionary/index`,
    );
});
...
test('surface default whose acom equivalent exists resolves correctly', () => {
    ...
    assert.equal(
        getParentReference('express', 'en_US'),
        `${ROOT}/acom/en_US/dictionary/index`,
    );
});
...
test('sandbox surface default inherits from acom', () => {
    ...
    assert.equal(
        getParentReference('sandbox', 'de_DE'),
        `${ROOT}/acom/de_DE/dictionary/index`,
    );
});
...
test('sandbox regional locale points to surface default', () => {
    ...
    assert.equal(
        getParentReference('sandbox', 'de_AT'),
        `${ROOT}/sandbox/de_DE/dictionary/index`,
    );
});
