...
test.describe('M@S Studio ACOM Full Pricing Express card CSS test suite', () => {
    ...
    // @studio-full-pricing-express-css-link-styles - Verify link styles
    test(`${features[10].name},${features[10].tags}`, async ({ page, baseURL }) => {
        ...
        await test.step('step-2: Check for link elements', async () => {
            // Check for legal link or phone link
            ...
            if (legalLinkCount > 0) {
                const legalLink = fullPricingExpress.cardLegalLink.first();
                await expect(legalLink).toBeVisible();
            }

            if (phoneLinkCount > 0) {
                const phoneLink = fullPricingExpress.cardPhoneLink.first();
    ...
    // @studio-full-pricing-express-css-responsive-layout - Verify responsive layout
    test(`${features[12].name},${features[12].tags}`, async ({ page, baseURL }) => {
        ...
        await test.step('step-2: Test desktop viewport', async () => {
            await page.setViewportSize({ width: 1440, height: 900 });
            const card = await studio.getCard(data.cardid);
            await expect(card).toBeVisible();
        });

        await test.step('step-3: Test tablet viewport', async () => {
            await page.setViewportSize({ width: 768, height: 1024 });
            const card = await studio.getCard(data.cardid);
            await expect(card).toBeVisible();
        });

        await test.step('step-4: Test mobile viewport', async () => {
    ...
    // @studio-full-pricing-express-css-card-sizing - Verify card sizing
    test(`${features[13].name},${features[13].tags}`, async ({ page, baseURL }) => {
        ...
        await test.step('step-2: Validate card dimensions', async () => {
            const card = await studio.getCard(data.cardid);
            ...
            // Verify card has dimensions (even if collapsed)
            expect(dimensions.width).toBeGreaterThan(0); // Card exists
            expect(dimensions.height).toBeGreaterThan(0); // Card exists
        });
    });

    // @studio-full-pricing-express-css-accessibility-contrast - Validate accessibility and contrast
    test(`${features[14].name},${features[14].tags}`, async ({ page, baseURL }) => {
        ...
        await test.step('step-2: Validate key elements are visible', async () => {
            // Title
            const titleCount = await fullPricingExpress.cardTitle.count();
            if (titleCount > 0) {
        ...
        await test.step('step-3: Validate focus states', async () => {
            const ctaCount = await fullPricingExpress.cardCTA.count();
            if (ctaCount > 0) {
                const ctaElement = fullPricingExpress.cardCTA.first();
