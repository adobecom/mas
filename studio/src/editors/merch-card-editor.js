...
import { countryTagLeafToLocaleCode } from './variation-utils.js';
...
class MerchCardEditor extends LitElement {
    ...
    #normalizeGroupedPreviewLocaleCode(tagValue) {
        const leaf = tagValue?.split('/').pop()?.trim();
        if (getLocaleByCode(leaf)) return leaf;
        const preferredLang = getLocaleByCode(Store.localeOrRegion())?.lang;
        return countryTagLeafToLocaleCode(leaf, Store.surface(), preferredLang);
    }
