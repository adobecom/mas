...
setup('authenticate, @mas-studio', async ({ page, baseURL, browserName }) => {
    ...
    await page.waitForURL((url) => url.href.startsWith(baseURL) && url.pathname.endsWith('/studio.html'));
    await expect(page).toHaveURL((url) => url.href.startsWith(baseURL) && url.pathname.endsWith('/studio.html'));
