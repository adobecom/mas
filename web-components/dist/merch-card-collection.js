...
`;function Ve(e){let r=td(e);return ed`
    ...
    `}import{html as Ni}from"./lit-all.min.js";var rd="(min-width: 1200px)",ad="(min-width: 600px)";function Po(e,r){if(!e)return"";let t=new DOMParser().parseFromString(`<picture>${e}</picture>`,"text/html");return r==="desktop"?t.querySelector(`source[media="${rd}"]`)?.getAttribute("srcset")??"":r==="tablet"?t.querySelector(`source[media="${ad}"]`)?.getAttribute("srcset")??"":r==="mobile"?t.querySelector("img")?.getAttribute("src")??"":""}var Mo=["tablet","mobile"];function id(e,r){let a=e.querySelector('[slot="backgrounds"]')?.innerHTML??"";Mo.forEach(i=>{let n=r.querySelector(`[data-backgrounds-breakpoint="${i}"]`);if(!n)return;n.replaceChildren();let o=Po(a,i);if(!o){n.textContent="\u2014";return}let s=document.createElement("img");s.loading="lazy",s.alt="",s.src=o,n.append(s)})}function Lt(e){return function(t){let a=e(),i=a.shadowRoot?.querySelector(".headless-backgrounds-detail");if(!i)return;let n=i.classList.contains("hidden");n&&id(a,i),i.classList.toggle("hidden",!n),t.target.textContent=n?"Show default only":"See all backgrounds"}}function Pt(e){return Ni`
        <button
            type="button"
